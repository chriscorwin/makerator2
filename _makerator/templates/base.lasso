<!DOCTYPE html >
<html>
	<head>
		<meta http-equiv="Content-Language" content="en">
		<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
		<!-- #{meta}# -->
		<title><!-- #{pageTitle}# --></title>
	</head>
	<body class="home" >
		<div id="container" class="container content">
			<div id="header" class="column span-16 first last">
				<a href="/" id="masthead" title=" #{pageTitle}# " class="loader"><strong><!-- #{pageTitle}# --></strong></a>
				<!-- #{navbarTop}# -->
			</div>
			
			
			<div id="primary" class="column span-16 first last">
				<div id="primaryContentWrapper">
					<div id="primaryContent" class="loader">
						<!-- #{primary}# -->
					</div>
			</div>
			
			
			<div id="" class="column span-16 first last">
				<div id="secondary" class="column span-8 first">
					<div id="secondaryContentWrapper">
						<div id="secondaryContent" class="loader">
							<!-- #{secondary}# -->
							<div id="errorContentWrapper">
								<div id="errorContent" class="loader">
									<!-- #{error}# -->
								</div>
							</div>
						</div>
					</div>
				</div>
				
				<div id="tertiary" class="column span-8 last">
					<div id="tertiaryContentWrapper">
						<div id="tertiaryContent" class="loader">
							<!-- #{tertiary}# -->
							<div id="debugContentWrapper">
								<div id="debugContent" class="loader">
									<!-- #{debug}# -->
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
			
			<div id="footer" class="column span-16 first last">
				<div id="footerContentWrapper">
					<div id="footerContent" class="loader">
						<!-- #{footer}# -->
						<!-- #{navbarFooter}# -->
					</div>
				</div>
			</div>
		</div>
	</body>
</html>

